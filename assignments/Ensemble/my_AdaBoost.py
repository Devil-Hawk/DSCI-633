...
class my_AdaBoost:
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators):
            ...
            # Train base classifier with sample training data
            ...
            # Compute error rate and alpha for estimator
            ...
            while error >= (1 - 1.0 / k):
                ...
                error = np.sum(diffs * w)
            # If one base estimator predicts perfectly we use that base estimator only
            if error == 0:
                self.alpha = [1]
                self.estimators = [self.estimators[i]]
                break
            # Compute alpha for estimator i and k for multiclass
            ...
            w /= np.sum(w)
        # Normalizing alpha
        self.alpha = np.array(self.alpha) / np.sum(self.alpha)
        return
    ...
    def predict(self, X):
        ...
        predictions = [self.classes_[np.argmax(prob)] for prob in probs.values]
        return predictions
    ...
    def predict_proba(self, X):
        ...
        for label in self.classes_:
            ...
            for j in range(len(self.estimators)):
                ...
                label_probs.append(alpha * (predictions == label))
            probs[label] = np.sum(label_probs, axis=0)
